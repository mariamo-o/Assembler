    ...
    def __second_pass(self) -> None:
        ...
        for i in range (lenght):
            if self.__asm[i][0]=='org':
                ...
                print (self.__bin)
                return
            elif len(self.__asm[i])>2 and str(self.__asm[i][1])=='hex':
                self.__bin[self.__format2bin(str(lc),'dec', 12)]= self.__format2bin(str(self.__asm[i][2]),'hex',16) 
                lc+=1
            elif len(self.__asm[i])>2 and str(self.__asm[i][1])=='dec':
                self.__bin[self.__format2bin(str(lc),'dec', 12)]= self.__format2bin(str(self.__asm[i][2]),'dec',16)
                lc+=1
            elif self.__islabel(self.__asm[i][0]):
                if self.__asm[i][1] in self.__mri_table.keys():
                    opcode=str(self.__mri_table[self.__asm[i][1]])
                    mode ='0'
                    address=str(self.__address_symbol_table[self.__asm[i][1]])
                    self.__bin[self.__format2bin(str(lc),'dec',12)]=mode+opcode+address
                    lc+=1
                elif self.__asm[i][1] in self.__rri_table.keys(): 
                    self.__bin[self.__format2bin(str(lc),'dec',12)] = str(self.__rri_table[self.__asm[i][1]]) 
                    lc+=1
                elif self.__asm[i][1] in self.__ioi_table.keys(): 
                    self.__bin[self.__format2bin(str(lc),'dec',12)] = str(self.__ioi_table[self.__asm[i][1]]) 
                    lc+=1    
            else:
                if self.__asm[i][0] in self.__mri_table.keys():
                    opcode=str(self.__mri_table[self.__asm[i][0]])
                    mode ='0'
                    address=str(self.__address_symbol_table[self.__asm[i][1]])
                    self.__bin[self.__format2bin(str(lc),'dec',12)]=mode+opcode+address
                    lc+=1
                elif self.__asm[i][0] in self.__rri_table.keys(): 
                    self.__bin[self.__format2bin(str(lc),'dec',12)] = str(self.__rri_table[self.__asm[i][0]]) 
                    lc+=1
                elif self.__asm[i][0] in self.__ioi_table.keys(): 
                    self.__bin[self.__format2bin(str(lc),'dec',12)] = str(self.__ioi_table[self.__asm[i][0]]) 
                    lc+=1         
